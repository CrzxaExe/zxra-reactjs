...
function Bzb() {
  const { img, act, des, name, ver, stats: { app, lang } } = jsonData.project.find(e => e.name === "Better Zxra Bedrock")
  ...
  return (
    <div className="mt-[2.25rem] lg:mt-[1.6rem] -z-1">
      ...
      <section id="home" className="w-full py-2 flex flex-wrap -mt-4 text-sm">
        <img src={img} className="w-full lg:w-[90%] mx-auto mt-4 rounded-[6px] block px-1 lg:px-6" alt={name} />

        <TextLine name="Project"/>
        ...
        <p className="px-2 lg:px-7 w-full text-xs lg:text-sm">
          ...
          <span className="block bg-nav rounded px-2 py-1"><i className='bx bx-note mr-1 text-sm' style={{color:'#ffffff'}}  ></i><p className="pb-3">{des}</p></span>
        </p>
      </section>
