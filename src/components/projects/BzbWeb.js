...
function BzbWeb() {
  const {
    ...
    ver,
    stats: { app, lang },
  } = jsonData.project.find((e) => e.name === "BZB Web");
    ...
    <div className="mt-[2.25rem] lg:mt-[1.6rem] -z-1 lg:h-[93vh] -mb-[6rem]">
      <header className="w-full pb-2 pt-4 lg:pt-7 flex lg:flex-col items-end lg:items-start mt-[0.5rem]">
        <div className="font-bold text-xl lg:text-2xl mr-3 text-tsec">
          <span className="lg:hidden">BZB - Web</span>
          <span className="hidden lg:block">BZB - Website</span>
        </div>
        <div className="before:content-['>'] lg:before:content-[''] before:mr-2 lg:before:mr-0 before:text-xl -mt-2 text-tter">
          Website lama dari BZB
        </div>
      </header>
          ...
          <p className="px-2 lg:px-7 w-full text-xs lg:text-sm">
            ...
            <span className="block bg-nav rounded px-2 py-1 whitespace-pre-wrap">
              <i
                ...
                style={{ color: "#ffffff" }}
              ></i>
              <p className="pb-1">{des}</p>
            </span>
          </p>
          <TextLine name="Tentang" />

          <p className="px-2 lg:px-7 w-full text-xs lg:text-sm">
            Website sederhana namun di styling dengan Tailwind CSS yang
            menampilkan tentang Addon Better Zxra Bedrock. Masih bisa diakses
            secara langsung namun sudah tidak pernah di update lagi selain
            sebagai tempat data download dari BZB
          </p>
        </div>
      </section>
    </div>
