...
function CzPortofolio() {
  const { img, act, des, name, ver, stats: { app, lang } } = jsonData.project.find(e => e.name === "Crzx Portofolio")
  ...
  return (
    <div className="mt-[2.25rem] lg:mt-[1.6rem] -z-1 lg:h-[93vh] -mb-[6rem]">
      <header className="w-full pb-2 pt-4 lg:pt-7 flex lg:flex-col items-end lg:items-start mt-[0.5rem]">
        <div className="font-bold text-xl lg:text-2xl mr-3 text-tsec"><span className="lg:hidden">Cz Porto</span><span className="hidden lg:block">Crzx Portofolio</span></div>
        <div className="before:content-['>'] lg:before:content-[''] before:mr-2 lg:before:mr-0 before:text-xl -mt-2 text-tter">Website portofolio Crzx</div>
      </header>
